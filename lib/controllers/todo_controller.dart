...
class TodoController extends GetxController {
  ...
  Future<void> fetchTodos() async {
    try {
      ...
      if (res.statusCode == 200) {
        ...
        Get.snackbar('Error', 'Failed to fetch todos - Status: ${res.statusCode}');
      }
    } catch (e) {
  ...
  Future<void> updateTodo(int id, {String? name, String? value, String? status}) async {
    try {
      isLoading(true);
      ...
      log('Updating todo $id with: $body');
      final res = await _api.put('/todo/$id', body);
      log('Update response: ${res.statusCode} - ${res.body}');
